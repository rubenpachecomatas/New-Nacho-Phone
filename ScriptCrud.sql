...
-- Table `sakila`.`TARIFA`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `sakila`.`TARIFA` (
  `idTar` INT NOT NULL,
...
-- Table `sakila`.`GASTOS`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `sakila`.`GASTOS` (
  `IdGas` INT(11) NOT NULL,
  `DNIUsu` VARCHAR(9) NOT NULL,
  `IdTar` INT(11) NOT NULL,
  `TfnGas` VARCHAR(45) NOT NULL,
  `MinGas` INT(11) NULL DEFAULT 0,
  `MegGas` INT(11) NULL DEFAULT 0,
  PRIMARY KEY (`IdGas`),
  INDEX `user_idx` (`DNIUsu` ASC),
  INDEX `idTar_idx` (`IdTar` ASC),
  CONSTRAINT `user`
    FOREIGN KEY (`DNIUsu`)
    REFERENCES `sakila`.`USUARIO` (`DNIUsu`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `idTar`
    FOREIGN KEY (`IdTar`)
    REFERENCES `sakila`.`TARIFA` (`idTar`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
